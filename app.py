...
def process_single_image(image: Image.Image, page_key: int) -> Optional[dict]:
...
st.write("Specialized for West Bengal registration documents with intelligent handwriting interpretation")
...
with col1:
    ...
    if image_to_display:
        try:
            st.image(image_to_display, use_container_width=True)  # Changed from use_column_width
...
with col2:
    ...
    if image_to_display and page_key not in st.session_state.processed_pages:
        if st.button(f"🚀 Process Page {page_key + 1}", key=f"process_{page_key}", type="primary"):
            with st.spinner("🤖 AI is analyzing the page... This may take 30-60 seconds."):
                progress_bar = st.progress(0)
                progress_bar.progress(25)
                
                extracted_data = process_single_image(image_to_display, page_key)
                ...
                if extracted_data and 'document_content' in extracted_data:
                    ...
                    st.success(f"✅ Successfully processed {len(entries)} entries!")
...
# --- Footer ---
st.write("---")
st.caption("🔧 Enhanced with West Bengal geographical knowledge and intelligent handwriting interpretation")